        ...
        <div class="order-list__box bgf">
          <div class="order-panel">
            <ul class="nav user-nav__title" role="tablist">
              <li role="presentation" :class="'nav-item '+(status === 0?'active':'')" @click="queryByStatus(0)"><a>所有订单</a></li>
              <li role="presentation" :class="'nav-item '+(status === 1?'active':'')" @click="queryByStatus(1)"><a>待付款 <span class="cr">0</span></a></li>
              <li role="presentation" :class="'nav-item '+(status === 2?'active':'')" @click="queryByStatus(2)"><a>待发货 <span class="cr">0</span></a></li>
              <li role="presentation" :class="'nav-item '+(status === 3?'active':'')" @click="queryByStatus(3)"><a>待收货 <span class="cr">0</span></a></li>
              <li role="presentation" :class="'nav-item '+(status === 4?'active':'')" @click="queryByStatus(4)"><a>待评价 <span class="cr">0</span></a></li>
            </ul>
            <div class="tab-content">
              <div class="tab-pane fade active in" id="all">
                <table class="table text-center">
                  ...
                  <tr v-if="page.total===0" class="order-empty"><td colspan="6">
                    <div class="empty-msg">最近没有任何订单，家里好像缺了点什么！<br><a>要不瞧瞧去？</a></div>
                  </td></tr>
                  <tr v-for="item in orders" class="order-item">
                    <td>
                      <label>
                        <a class="num">
                    ...
                    <td class="state">
                      ...
                      <a class="but c9" @click="orderDetailPage(item.orderId)">订单详情</a>
                    </td>
                    <td class="order">
                      <div class="del"><i class="el-icon-delete"></i></div>
                      <a v-if="status === 1" class="but but-primary" @click="toPay">立即付款</a>
                      <a v-if="status === 3" class="but but-primary" @click="receipt">确认收货</a>
                      <a v-if="status === 4" class="but but-link">评价</a>
                      <a v-if="status === 1" class="but c3">取消订单</a>
                      <a v-if="status !== 1" class="but c3" @click="returnPage">退款/退货</a>
                    </td>
                  </tr>
                  </tbody>
                </table>
...
<script>
  import {pageOrders} from "../../../../api/user";
  ...
  export default {
    ...
    methods: {
      ...
      getData(){
        this.queryMyProducts(this.page.currentPage,this.page.pageSize,this.status)
      },
      ...
      queryMyProducts(currentPage,pageSize,status){
        ...
        this.loading = true;
        pageOrders({currentPage:currentPage,pageSize:pageSize,status: status,id:userInfo.id}).then(res => {
          this.loading = false;
          if(res.status === 200){
      ...
      orderDetailPage(orderId){//订单详情
        this.$router.push({path: '/personal/orderDetail',query:{orderId:orderId}},);
      },
      toPay(){//订单支付
